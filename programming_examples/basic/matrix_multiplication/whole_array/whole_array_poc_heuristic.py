...
from aie.helpers.dialects.ext.scf import _for as range_     #range_ is used for loops inside the hw
from aie.helpers.taplib import TensorAccessPattern, TensorAccessSequence

from aie.iron import str_to_dtype

# This is important becasue it defines the microkernel MAC dimensions for different data types and devices.
# r, s, t correspond to the blocking factors in M, K, N dimensions respectively
# If we use npu2 with i8, blocks must be multiples of (8, 8, 8) otherwise the microkernel instructions will not perform at the maximum efficiency.
...
def main():
    # Argument parsing from command line or makefile
    ...
    argparser.add_argument("--emulate-bf16-mmul-with-bfp16", type=bool, default=False)  # Only applicable to npu2 bf16
    ...
    with mlir_mod_ctx() as ctx:
        ...
        print(ctx.module)   # Print the MLIR module to stdout
...
def my_matmul(
    ...
    def device_body():
        ...
        zero = external_func(f"zero{scalar_suffix}_{dtype_out_str}", inputs=[C_l1_ty]) # clear output tile before accumulation
        matmul = external_func(
            f"matmul{scalar_suffix}_{dtype_in_str}_{dtype_out_str}",
            inputs=[A_l1_ty, B_l1_ty, C_l1_ty],     # MAC and it loads the correct kernel (for instance matmul_i16_i16)
        )
        ...
        # Input A
        # L3 -> L2 data movement (shim to mem)
        ...
        # L2 -> L1 data movement, broadcast along rows and data layout transformation
        for row in range(n_aie_rows):
            A_l2l1_fifos[row] = object_fifo(
                ...
                A_l1_ty,
                (
                    [   # Data layout transformation for A in order to match microkernel requirements
                        (m // r, r * k),                    # macro-tiling for the rows. After processing r rows, the DMA must jump r*k elements to get to the next block of r rows          
                        (k // s, s),                        # same for the columns
                        (r, k),                             # micro-tiling for the rows. Jump k elements after processing r rows
                        (s, 1),                             # micro-tiling for the columns
                    ]
                    if not use_scalar
                    else []
                ),
        ...
        # The runtime sequence function for data movement to/from AIE array
        def sequence(A, B, C):
            # We are limited in the number of Buffer Descriptors (BDs). After synchronizing, we can reuse BDs.
                ...
                for pingpong in [0, 1]:
                    ...
                    for col in range(n_aie_cols):
                        ...
                        for tile_row in range(tb_n_rows):
                            ...
                            curr_A_sizes = [N // n // n_aie_cols, K // k, m * n_A_tiles_per_shim, k]
                            curr_A_strides = [0, k, K, 1]
